...
import { responseError } from "./common_func.js";
...
export const verifyToken = (ctx) => {
    ...
    if (!token) {
        log.warn("Not found token in request - ", ctx.url);
        responseError(ctx, 401, "TOKEN_REQUIRED", "Token is needed for the request.");
        return false;
    }
